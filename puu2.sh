...
echo "                PUU2"
echo "         Written by Kevin Ross"
echo "    in collaboration with Joe Garcia"
echo "       for CyberArk Impact 2017"

printf "\n"


read -p "Enter the CyberArk URL (ie: https://components.cyberark.local) : " CAURL
read -p "Enter Your UserName: " CAUsername
...
curl --insecure -s -i -H "$AUTH" -H "Content-Type:application/json" -X POST --data '{"account" : {"safe":"'"$CYBRSafe"'","platformID": "'"$CYBRPlatformID"'","address": "'"$CYBRAddress"'","accountName": "'"$CYBRObjectName"'","password": "'"$CYBRPassword"'","username": "'"$CYBRUsername"'",}}' $CAURL/PasswordVault/WebServices/PIMServices.svc/Account | grep HTTP/1.1 | tee puu2.log
...
#logoff_from_safe
curl --insecure -s -i -H "$AUTH" -d -H "Content-Type:application/json" -X POST $CAURL/PasswordVault/WebServices/auth/Cyberark/CyberArkAuthenticationService.svc/Logoff| grep HTTP/1.1 | tee puu2.log
printf "\nLogged off\n"
